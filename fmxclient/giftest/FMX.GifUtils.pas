...
const
  alphaTransparent = $00;
  // Every color in a graphic has an alpha channel value to indicate how transparent it is
...
{$ENDIF});
  ...
  private
    FImage: TImage;
    FGifFrameList: TGifReader.TGifFrameList;
    FTimer: TTimer;
...
type
  ...
  TInternalColor = TGifReader.TInternalColor;
{$POINTERMATH ON}
...
begin
  if Dest.Map(TMapAccess.Write, DestData) then
    try
      if Source.Map(TMapAccess.Read, SrcData) then
        try
          ...
          if DestX + SrcRect.Width > Dest.Width then
            SrcRect.Width := Dest.Width - DestX;
          if DestY + SrcRect.Height > Dest.Height then
            SrcRect.Height := Dest.Height - DestY;

          if (SrcRect.Left < SrcRect.Right) and
            (SrcRect.Top < SrcRect.Bottom) then
          begin
            for I := 0 to SrcRect.Height - 1 do
            begin
              lpColorSrc := SrcData.GetPixelAddr(SrcRect.Left, SrcRect.Top + I);
...
var
  ...
  procedure RenderFrame(const Index: Integer;
    const aFrames: array of TGifFrameItem; const aDisplay: TBitmap);
  var
  ...
  begin
      ...
      while First > 0 do
      begin
        if (FScreenWidth = aFrames[First].Bitmap.Width) and
          (FScreenHeight = aFrames[First].Bitmap.Height) then
        begin
          if (aFrames[First].FDisposalMethod = GIF_DISPOSAL_BACKGROUND) and
            (First < Last) then
            Break;
        end;
        Dec(First);
      ...
      begin
        case aFrames[I].FDisposalMethod of
          GIF_DISPOSAL_UNSPECIFIED, GIF_DISPOSAL_LEAVE:
            begin
          ...
          GIF_DISPOSAL_BACKGROUND:
            ...
            begin
              ...
              aDisplay.ClearRect(TRectF.Create(aFrames[I].FPos.x,
                aFrames[I].FPos.Y, aFrames[I].FPos.x + aFrames[I].Bitmap.Width,
      ...
      MergeBitmap(aFrames[Index].Bitmap, aDisplay, aFrames[Index].Bitmap.Bounds,
        aFrames[Index].FPos.x, aFrames[Index].FPos.Y);
  end;
...
destructor TGifReader.TGifFrameItem.Destroy;
begin
...
begin
    ...
    FGifFrameList := TGifReader.TGifFrameList.Create();
